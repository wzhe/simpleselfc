...
void shownode(struct ASTnode* node,
    ...
    printf("%s%s", prefix, asttypestr(node->op));
    if (node->op == A_INTLIT) {
    ...
    if (havechild(node)){
        ...
        if (node->left) shownode(node->left, buf, bufchild);
        if (node->mid) shownode(node->mid, buf, bufchild);
        if (node->right) shownode(node->right, buf, bufchild);
        free(buf);
        free(bufchild);
    }
}
